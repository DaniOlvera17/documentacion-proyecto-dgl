---
layout: default
title: Historias de Usuario
---

# 🧩 Historias de Usuario

## Formato Utilizado

**Como** [rol]  
**Quiero** [función]  
**Para** [beneficio]

---

## Historias del Backlog

### 📌 Gestión de Inventarios

- **HU-01**  
  Como operador de almacén  
  Quiero registrar materiales  
  Para mantener el inventario actualizado

- **HU-02**  
  Como operador  
  Quiero consultar el inventario  
  Para conocer las existencias disponibles

---

### 📦 Gestión de Embarques

- **HU-03**  
  Como supervisor  
  Quiero crear órdenes de embarque  
  Para enviar materiales de forma controlada

- **HU-04**  
  Como operador  
  Quiero validar embarques con QR  
  Para reducir errores de envío

---

### 📊 Reportes

- **HU-05**  
  Como gerente  
  Quiero ver reportes de inventario  
  Para tomar decisiones estratégicas

---

### 🔐 Seguridad

- **HU-06**  
  Como administrador  
  Quiero gestionar usuarios y roles  
  Para controlar accesos al sistema

---

## Priorización MoSCoW

| Historia | Prioridad |
|----------|------------|
| HU-01 | Must Have |
| HU-02 | Must Have |
| HU-03 | Must Have |
| HU-04 | Should Have |
| HU-05 | Should Have |
| HU-06 | Must Have |

---

[← Volver al Análisis de Negocio](index)

<div class="text-center text-gray">
  <small>Última actualización: 06 de diciembre de 2025</small><br>
  <small>© 2025 Diesel Global Logistics Inc.</small>
</div>