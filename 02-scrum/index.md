---
layout: default
title: Scrum - Sprints del Proyecto
---

# 🧩 Marco de Trabajo SCRUM

Este apartado documenta la gestión ágil del proyecto **Plataforma Web de Gestión de Almacén y Embarques – Diesel Global Logistics Inc.**, utilizando la metodología **SCRUM**.

---

## 📌 Información General

| Elemento | Descripción |
|----------|-------------|
| Metodología | SCRUM |
| Duración total | 7 Sprints |
| Lugar de desarrollo | Querétaro, México |
| Herramienta de gestión | Jira |
| Reuniones diarias | Daily Scrum (15 minutos) |

---

## 🚀 Sprints del Proyecto

| Sprint | Nombre | Duración | Enlace |
|--------|--------|----------|--------|
| 🟢 Sprint 1 | Gestión y Planificación | 1 semana | [Ver Sprint 1](sprint-1) |
| 🔵 Sprint 2 | Definición y Análisis de Requisitos | 1 semana | [Ver Sprint 2](sprint-2) |
| 🟠 Sprint 3 | Diseño del Sistema | 2 semanas | [Ver Sprint 3](sprint-3) |
| 🟣 Sprint 4 | Desarrollo Backend | 2 semanas | [Ver Sprint 4](sprint-4) |
| 🟡 Sprint 5 | Desarrollo Frontend e Integración | 1 semana | [Ver Sprint 5](sprint-5) |
| 🔴 Sprint 6 | Pruebas y Validaciones | 2 semanas | [Ver Sprint 6](sprint-6) |
| ⚫ Sprint 7 | Implementación, Capacitación y Cierre | 2 semanas | [Ver Sprint 7](sprint-7) |

---

## 🧑‍🤝‍🧑 Equipo SCRUM

| Rol | Nombre |
|-----|--------|
| Gestor de Proyecto / Scrum Master | Ángel Elías Romero Paulin |
| Analista | José Pedro Cruz Ramírez |
| Diseñadores UI/UX | Luis Fernando Loma Navarrete / María Alondra Ortiz Benítez |
| Desarrollador | Esteban Reséndiz Urquiza |
| QA | Raúl Mejía Reséndiz |
| Tester | Roberto Joshua García Solís / Omar Garcia Perrusquia|
| Documentador | Daniel Olvera Romero |

---

## 🔄 Ceremonias SCRUM

| Ceremonia | Frecuencia | Duración |
|----------|------------|----------|
| Sprint Planning | Inicio de cada sprint | 2-4 horas |
| Daily Scrum | Diario | 15 minutos |
| Sprint Review | Fin de cada sprint | 1-2 horas |
| Sprint Retrospective | Fin de cada sprint | 1-2 horas |

---

## 📊 Artefactos SCRUM

- Product Backlog
- Sprint Backlog
- Incremento de producto
- Burndown Chart
- Definition of Done (DoD)

---

[← Volver al índice principal](../index)
